# Project conventions for Codex sessions

When you create git commits in this repository:

- **Do NOT include a `Co-Authored-By: Codex ...` trailer.**
  The maintainer prefers commits to show a single human author. The
  default Codex commit recipe adds this trailer; override it
  for this repo.
...
- Never push directly to `main` from a Codex session unless the
  maintainer has explicitly asked for it in the current turn.
  Branch + PR is the default path.

These rules apply to every Codex session that operates on this
